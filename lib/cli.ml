...
let cli_parser =
    ...
    and+ cut_area = named_with_default ["cut"] string ~default:"" ~doc:"Cut area from images. Format: \"x y width height\""
    and+ words = pos_all string ~doc:"File name or diretory name" in
    Cli_args.create ~words ~output_image_quality ~input_from_file ~input_from_dir ~keep_temp ~overwrite ~trim ~cut_area:(Cli_args.Area.create_from_string cut_area)
